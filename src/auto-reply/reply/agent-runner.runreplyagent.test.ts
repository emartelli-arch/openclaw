...
describe("runReplyAgent typing (heartbeat)", () => {
  ...
  it("does not retry transient HTTP failures", async () => {
    vi.useFakeTimers();
    let calls = 0;
...
describe("runReplyAgent memory flush", () => {
  ...
  it("does not run a separate memory flush turn", async () => {
    await withTempStore(async (storePath) => {
      const sessionKey = "main";
      const sessionEntry = {
        sessionId: "session",
  ...
  it("does not increment compaction count from disabled memory flush", async () => {
    await withTempStore(async (storePath) => {
      const sessionKey = "main";
      const sessionEntry = {
        sessionId: "session",
